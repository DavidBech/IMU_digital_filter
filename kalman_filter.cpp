...
kalman_filter::kalman_filter(){
  ...
  P_Error_Cov(0,2) = 0.0f;
	P_Error_Cov(1,0) = 0.0f;
	P_Error_Cov(1,1) = 0.0f;
  P_Error_Cov(1,2) = 0.0f;
  P_Error_Cov(2,0) = 0.0f;
  P_Error_Cov(2,1) = 0.0f;
  P_Error_Cov(2,2) = 0.0f;

	Q_Cov_Noise(0,0) = 0.01f;
	Q_Cov_Noise(0,1) = 0.00f;
  Q_Cov_Noise(0,2) = 0.00f;
	Q_Cov_Noise(1,0) = 0.00f;
	Q_Cov_Noise(1,1) = 0.01f;
  Q_Cov_Noise(1,2) = 0.00f;
  Q_Cov_Noise(2,0) = 0.00f;
  Q_Cov_Noise(2,1) = 0.00f;
  Q_Cov_Noise(2,2) = 0.01f;
...
void kalman_filter::calc_F_state_trans(){
 ...
 F_state_trans(0, 0) = 1;
 F_state_trans(0, 1) = sinf(X_state_estimate(0))*tan(X_state_estimate(1));
 F_state_trans(0, 2) = cosf(X_state_estimate(0))*tan(X_state_estimate(1));
 F_state_trans(1, 0) = 0.0f;
 F_state_trans(1, 1) = cosf(X_state_estimate(0));
 F_state_trans(1, 2) = -sinf(X_state_estimate(0));
 F_state_trans(2, 0) = 0.0f;
 F_state_trans(2, 1) = sinf(X_state_estimate(0))/cosf(X_state_estimate(1));
 F_state_trans(2, 2) = cosf(X_state_estimate(0))/cosf(X_state_estimate(1));
}

void kalman_filter::calc_Jacobian_of_F(BLA::Matrix<3> *Gyro_Data, BLA::Matrix<3,3>*A_in){
	BLA::Matrix<3> &GD = *Gyro_Data;
	BLA::Matrix<3,3> &A = *A_in;
  ...
  A(0,0) = tan_the*(GD(1)*cos_phi-GD(2)*sin_phi); // d phi / d phi
  A(0,1) = (GD(1)*sin_phi + GD(2)*cos_phi) * sec_the * sec_the; // d phi / d theta
  A(0,2) = 0.0f; // d phi / d psi
  A(1,0) = -GD(1)*sin_phi - GD(2)*cos_phi; // d theta / d phi
  A(1,1) = 0.0f; // d theta / d theta
  A(1,2) = 0.0f; // d theta / d psi
  A(2,0) = sec_the*(GD(1)*cos_phi - GD(2)*sin_phi); // d theta / d phi
  A(2,1) = sec_the*tan_the*(GD(1)*sin_phi + GD(2)*cos_phi); // d theta / d theta
  A(2,2) = 0.0f; // d theta / d psi
...
void kalman_filter::update(BLA::Matrix<3> *Accel_Data){
	...
	BLA::Matrix<3,3> Kalman_Gain = P_times_C_trans * Inverse_Comp;
...
void kalman_filter::calc_h_Jacob(BLA::Matrix<3> *Accel_Data, BLA::Matrix<3,3>*C_h_Jacob, BLA::Matrix<3>*transformed_measure_h){
	BLA::Matrix<3,3> &C = *C_h_Jacob;
	BLA::Matrix<3> &h = *transformed_measure_h;
	float sin_phi = sinf(X_state_estimate(0));
	float cos_phi = cosf(X_state_estimate(0));
	float sin_the = sinf(X_state_estimate(1));
	float cos_the = cosf(X_state_estimate(1));
	C(0,0) = 0.0f; // d h0 / d phi
	C(0,1) = cos_the;  // d h0 / d theta
  C(0,2) = 0.0f; // d h0 / d psi
	C(1,0) = -cos_phi * cos_the; // d h1 / d phi
	C(1,1) = sin_phi * sin_the; // d h1 / d theta
  C(1,2) = 0.0f;            // d h1 / d psi
	C(2,0) = sin_phi * cos_the; // d h2 / d phi
	C(2,1) = cos_phi * sin_the; // d h2 / d theta
  C(2,2) = 0.0f;              // d h2 / d psi
  C *= G_MPS2;
...
float kalman_filter::get_yaw_rad(){
...
float kalman_filter::get_yaw_deg(){
