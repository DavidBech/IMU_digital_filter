...
class kalman_filter{
    ...
    void calc_Jacobian_of_F(BLA::Matrix<3> *Gyro_Data, BLA::Matrix<3,3>*A);
    void calc_h_Jacob(BLA::Matrix<3> *Accel_Data);
    void calc_h_Jacob(BLA::Matrix<3> *Accel_Data, BLA::Matrix<3,3>*C_h_Jacob, BLA::Matrix<3> *transformed_measure_h);
	...
	/* 1   sin(phi)*tan(theta) cos(phi)*tan(theta)
	   0   cos(phi)             -sin(phi)*/
	BLA::Matrix<3, 3> F_state_trans;

	/* (0)=phi/roll 
	   (1)=theta/pitch */
	BLA::Matrix<3> X_state_estimate;

    // Covariance Matrix
	BLA::Matrix<3, 3> P_Error_Cov;

    // Process and measurement noise
	BLA::Matrix<3, 3> Q_Cov_Noise; 
